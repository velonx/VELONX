...
export default function NetworkProfileClient({
  ...
  // Seeded from the server so the profile's real content is in the initial HTML
  // (crawlable, no skeleton flash). The fetch below layers on the viewer-specific
  // bits the server render can't know: connection status and mutual connections.
    ...
    async function fetchProfile() {
      try {
        ...
        if (data.success) {
          ...
          // Swap an ObjectId URL for the readable slug. Purely cosmetic now —
          // the server render already emits a slug canonical for crawlers.
          if (data.data.slug && userId !== data.data.slug) {
            window.history.replaceState({}, "", `/network/${data.data.slug}`);
          }
        }
        // A failure here leaves the server-seeded profile on screen; only the
        // viewer-specific connection state is missing, so don't blank the page.
      } catch {
        toast.error("Failed to load the latest profile details");
      }
    }
