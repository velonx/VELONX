...
export default function GroupDetailClient({
  ...
  const { groups, isLoading, joinGroup, requestJoinGroup, leaveGroup, updateGroup, deleteGroup, refetch, memberGroupIds } = useCommunityGroups(
    initialGroup ? [initialGroup] : undefined
  );
  const group = groups?.find(g => g.id === groupId) || initialGroup;
  ...
  const {
    ...
    approveRequest,
    rejectRequest
  } = useGroupMembers(groupId, initialMembers);

  const { posts, isLoading: postsLoading, createPost, isCreating, loadMore, hasMore, refetch: refetchPosts } = useCommunityPosts({
    groupId,
          ...
          <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex-1">
              <div className="flex items-center gap-3 mb-2">
                <h1 className="heading-section text-3xl">{group.name}</h1>
